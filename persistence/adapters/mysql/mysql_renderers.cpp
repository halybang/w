...
namespace persistence {
    std::string MySQLQueryRenderer::render(const SelectQuery& x) {
      MySQLValueRenderer renderer{conn, symbolic_relation_resolver};
      std::stringstream ss;
      ss << "SELECT ";
      if (x.select.size()) {
    ...
    std::string MySQLQueryRenderer::render(const UpdateQuery& x) {
      return "NIY";
    }

    std::string MySQLQueryRenderer::render(const DeleteQuery& x) {
      return "NIY";
    }

    std::string MySQLQueryRenderer::render(const InsertQuery& x) {
      std::stringstream ss;
      ...
      ss << ") VALUES (";
      MySQLValueRenderer vr { conn, symbolic_relation_resolver };
    ...
    std::string MySQLValueRenderer::render(const StarFrom& x) {
      return wayward::format("{0}.*", x.relation);
    }

    std::string MySQLValueRenderer::render(const StringLiteral& x) {
      return wayward::format("'{0}'", conn.sanitize(x.literal));
    }

    std::string MySQLValueRenderer::render(const NumericLiteral& x) {
      // TODO: Something cleverer once NumericLiteral becomes more aware of number types
      return wayward::format("{0}", x.literal);
    }

    std::string MySQLValueRenderer::render(const BooleanLiteral& x) {
      return x.value ? "'t'" : "'f'";
    }

    std::string MySQLValueRenderer::render(const ColumnReference& x) {
      return wayward::format("{0}.{1}", x.relation, x.column);
    }

    std::string MySQLValueRenderer::render(const ColumnReferenceWithSymbolicRelation& x) {
      return wayward::format("{0}.{1}", symbolic_relation_resolver.relation_for_symbol(x.relation), x.column);
    }

    std::string MySQLValueRenderer::render(const Aggregate& x) {
      std::stringstream ss;
      ss << x.function << '(';
    ...
    std::string MySQLValueRenderer::render(const List& x) {
      std::stringstream ss;
      ss << '(';
    ...
    std::string MySQLValueRenderer::render(const CaseSimple& x) {
      return "NIY";
    }

    std::string MySQLValueRenderer::render(const Case& x) {
      return "NIY";
    }

    std::string MySQLValueRenderer::render(const NotCondition& x) {
      return wayward::format("NOT ({0})", x.subcondition->to_sql(*this));
    }

    std::string MySQLValueRenderer::render(const UnaryCondition& x) {
      std::string op;
      switch (x.op) {
    ...
    std::string MySQLValueRenderer::render(const BinaryCondition& x) {
      std::string op;
      switch (x.op) {
    ...
    std::string MySQLValueRenderer::render(const BetweenCondition& x) {
      return wayward::format("{0} IS BETWEEN {1} AND {2}", x.value->to_sql(*this), x.lower_bound->to_sql(*this), x.upper_bound->to_sql(*this));
    }

    std::string MySQLValueRenderer::render(const LogicalCondition& x) {
      std::string op;
      switch (x.op) {
